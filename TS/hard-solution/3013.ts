...
// Simple PQ3013 implementation for TypeScript
class PQ3013<T> {
    private data: T[];
...
function minimumCost(nums: number[], k: number, dist: number): number {
    ...
    // Use a PQ3013 class with custom comparator for max-heap and min-heap
    const heapUsed = new PQ3013<[number, number]>((a, b) => b[0] - a[0]); // max-heap

    const heapUnused = new PQ3013<[number, number]>((a, b) => a[0] - b[0]); // min-heap
