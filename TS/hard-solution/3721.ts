...
function longestBalancedII(nums: number[]): number {
    const n = nums.length;
    ...
    const update = (
        node: number, segLeft: number, segRight: number,
        queryLeft: number, queryRight: number, addValue: number
    ): void => {
    ...
    const findFirst = (node: number, segLeft: number, segRight: number, limit: number): number => {
        if (segLeft > limit) 
            return -1;
