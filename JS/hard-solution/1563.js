...
var stoneGameV = function(stoneValue) {
    ...
    const search = (leftBound, rightBound) => {
        const total = prefix[rightBound + 1] - prefix[leftBound];
        ...
        while (left < right) {
            ...
            const leftSum = prefix[mid + 1] - prefix[start];
    ...
    for (let length = 1; length < n; length++) {
        for (let i = 0; i < n - length; i++) {
            ...
            if (leftHalf * 2 === total) {
                ...
                const leftBest = k === i ? 0 : left[i][k - 1];

                const rightBest = k === j ? 0 : right[k + 1][j];
